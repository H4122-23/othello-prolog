...
play :-
  ...
  clear(),
  statistics(walltime, [_, _]),
  play(Grid, x, d, 4, 0, false),!.

play(Grid, Player, Alpha_last, Index_last, Turn, Auto):-
  ...
  playerInput(Grid, Alpha_last, Index_last, Alpha, Index, Player, Turn, Auto),
  makeAMove(Grid, Player, Index, Alpha, Turn, Auto).  %? always Match \w side effect Display
...
play(Grid, Player, Alpha_last, Index_last, Turn, Auto):-
  ...
  play(Grid, Player2, Alpha_last, Index_last, Turn, Auto).
...
% (test if Valid move)
makeAMove(Grid, Player, Index, Alpha, Turn, Auto) :-
  ...
  NextTurn is Turn + 1,
  play(Grid_out, NextPlayer, Alpha, Index, NextTurn, Auto).
...
makeAMove(Grid, Player, Index, Alpha, Turn, Auto) :-
  ...
  play(Grid, Player, Alpha, Index, Turn, Auto).



% let the system make the play or the user (defined by prompt:option/2)
playerInput(Grid, Alpha_last, Index_last, Alpha, Index, Player, _, _) :-
...
% auto play random 5 first moves
playerInput(Grid, Alpha_last, Index_last, Alpha, Index, Player, Turn, true) :-
  Turn =< 5,
  playerType(Player, _), % algorithm don't matter
  afficheCellule(Player), write('- Random'),nl,
  afficheGrille(Grid, Alpha_last, Index_last),
  inputPickRandomCoord(Grid, Alpha, Index, Player)
.
...
% auto play
playerInput(Grid, Alpha_last, Index_last, Alpha, Index, Player, Turn, true) :-
  % after 5 moves, contiue as normal
  playerInput(Grid, Alpha_last, Index_last, Alpha, Index, Player, Turn, false)
.
